...
class GeneralCard extends Component {
  render() {
    return (
      <div
        onClick={() => this.props.OpenModal(this)}
        className={"general-card " + this.props.classBorder}
      >
        ...
        <span className="general-number primary">
          {formatNumberK(this.props.generalNumber)}
        </span>
        <span className="general-desc">{this.props.generalDesc}</span>
        <span className={"general-daily status-" + this.props.status}>
          {this.props.todayNumber} Today
        </span>
      </div>
    );
  }
